...
echo "Searching Google for 'DevOps' in Spain (MaxResults 5) against $URL"
...
PAYLOAD='{"Query": "DevOps", "Location": "Spain", "MaxResults": 5, "Sources": ["Google"]}'
...
if echo "$RESPONSE" | grep -q "\["; then
  ...
  echo "Found $COUNT jobs from Google."
  if [ "$COUNT" -eq 0 ]; then
