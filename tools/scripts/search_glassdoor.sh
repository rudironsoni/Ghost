...
echo "Searching Glassdoor for 'Data Engineer' (Remote) (MaxResults 5) against $URL"
...
PAYLOAD='{"Query": "Data Engineer", "Location": "Remote", "MaxResults": 5, "Sources": ["Glassdoor"]}'
...
if echo "$RESPONSE" | grep -q "\["; then
  ...
  echo "Found $COUNT jobs from Glassdoor."
  if [ "$COUNT" -eq 0 ]; then
