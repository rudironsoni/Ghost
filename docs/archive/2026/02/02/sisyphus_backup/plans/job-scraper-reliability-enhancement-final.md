# Job Scraper Reliability Enhancement - Final Work Plan
...
- Use existing SessionFactory infrastructure (currently underutilized by scrapers)
...
1. **Enhanced Session Management Framework**
   - All platforms use SessionFactory for consistent session management
...
**What to do**:
- [ ] Modify `IndeedApiClient` constructor to accept `SessionFactory` via DI
- [ ] Replace per-request HttpClient creation with `sessionFactory.CreatePlatformSession("indeed").ExecuteAsync(() => request)`
- [ ] Remove manual cookie header injection, use CookieContainer from session
- [ ] Wire `IndeedOptions.DelayMinMs`/`DelayMaxMs` into session jitter
- [ ] Add User-Agent rotation through session
...
**References**:
- Pattern: `/src/Platforms/Ghost.Platform.Common/Session/SessionFactory.cs` - Session factory for platform-specific sessions
- Pattern: `/src/Platforms/Ghost.Platform.Common/Session/RotatingProxySession.cs` - Session implementation with proxy rotation
...
**Acceptance Criteria**:
- [ ] `IndeedApiClient` can be constructed with `SessionFactory`
...
**Test Plan**:
```csharp
// Test: SessionFactory integration
[Fact]
public async Task SearchAsync_UsesSessionFactory()
{
    var mockFactory = new Mock<SessionFactory>(MockBehavior.Strict, proxyProvider, logger);
    var mockSession = new Mock<RotatingProxySession>(MockBehavior.Strict, proxyProvider, httpClient, options);
    mockFactory.Setup(f => f.CreatePlatformSession("indeed")).Returns(mockSession.Object);
    var client = new IndeedApiClient(mockFactory.Object, options, logger);
...
- Message: `feat(indeed): integrate SessionFactory for session management`
...
**What to do**:
- [ ] Modify `GlassdoorApiClient` to use `SessionFactory`
- [ ] Update `GlassdoorBrowserClient` to use session factory for session creation
- [ ] Unify CSRF token extraction between API and browser clients
- [ ] Remove hardcoded fallback token usage
- [ ] Add proper cookie persistence through sessions
...
**References**:
- Shared: `/src/Platforms/Ghost.Platform.Common/Session/SessionFactory.cs` - Session factory
...
**Acceptance Criteria**:
- [ ] `GlassdoorApiClient` uses SessionFactory
- [ ] CSRF token extraction unified (single source of truth)
- [ ] Hardcoded fallback token marked obsolete with warning
- [ ] Cookie container persists across API and browser through sessions
- [ ] Existing tests pass

**Commit**:
- Message: `feat(glassdoor): integrate SessionFactory and unify CSRF extraction`
...
**What to do**:
- [ ] Modify `GoogleJobsApiClient` to accept SessionFactory
- [ ] Remove manual Cookie header injection
- [ ] Ensure User-Agent consistency with Sec-CH-UA headers through sessions
- [ ] Add browser escalation path when consent detected
- [ ] Fix async pagination to use sessions
...
**References**:
- Session Factory: `/src/Platforms/Ghost.Platform.Common/Session/SessionFactory.cs` - Session factory
...
**Acceptance Criteria**:
- [ ] CookieContainer used instead of manual headers through sessions
- [ ] User-Agent rotation updates all Sec-CH headers through sessions
- [ ] Browser escalation works when consent detected
- [ ] Async pagination uses sessions
- [ ] Existing tests pass

**Commit**:
- Message: `feat(google): integrate SessionFactory and fix cookie handling`
