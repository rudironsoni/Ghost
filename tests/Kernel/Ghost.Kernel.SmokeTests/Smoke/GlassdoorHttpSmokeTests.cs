...
/// <summary>
/// HTTP-based smoke tests for Glassdoor platform.
/// Tests the Ghost API endpoints for Glassdoor job search and retrieval.
/// </summary>
[Trait("Category", "Smoke")]
[Trait("Plugin", "Glassdoor")]
public class GlassdoorHttpSmokeTests : ReliabilityTestBase, IClassFixture<GhostWebApiFixture>
{
    private readonly GhostWebApiFixture _fixture;

    public GlassdoorHttpSmokeTests(GhostWebApiFixture fixture, ITestOutputHelper output) : base(output)
    {
        _fixture = fixture;
    }
    ...
    public async Task SearchJobs_Returns_Populated_Fresh_Data()
    {
        ...
        // Act
        Output.WriteLine($"Searching Glassdoor via API for: {searchRequest.query}");
    ...
    public async Task SearchJobs_WithLocation_Returns_Jobs_In_Location()
    {
        ...
        // Act
        Output.WriteLine($"Searching Glassdoor via API for: {searchRequest.query} in {searchRequest.location}");
        List<JobListing>? results = await _fixture.PostAsync<object, List<JobListing>>(
            "/api/jobs/search",
            searchRequest,
            Output);
    ...
    public async Task GetJobDetails_ById_Returns_Valid_Data()
    {
        ...
        jobDetails.Source.Should().Be("Glassdoor", "source should be Glassdoor");

        // Validate required fields
        jobDetails.AssertRequiredFields();
        jobDetails.AssertValidPlatformId("Glassdoor");
