...
[Trait("Plugin", "InfoJobs")]
public class InfoJobsHttpSmokeTests : ReliabilityTestBase, IClassFixture<GhostWebApiFixture>
{
    private readonly GhostWebApiFixture _fixture;

    public InfoJobsHttpSmokeTests(GhostWebApiFixture fixture, ITestOutputHelper output) : base(output)
    {
        _fixture = fixture;
    }
    ...
    public async Task SearchJobs_Returns_Populated_Fresh_Data()
    {
        ...
        // Act
        Output.WriteLine($"Searching InfoJobs via API for: {searchRequest.query}");
    ...
    public async Task SearchJobs_WithLocation_Returns_Jobs_In_Location()
    {
        ...
        // Act
        Output.WriteLine($"Searching InfoJobs via API for: {searchRequest.query} in {searchRequest.location}");
        List<JobListing>? results = await _fixture.PostAsync<object, List<JobListing>>(
            "/api/jobs/search",
            searchRequest,
            Output);
