...
/// <summary>
/// HTTP-based smoke tests for LinkedIn platform.
/// Tests the Ghost API endpoints for LinkedIn job search and retrieval.
/// </summary>
[Trait("Category", "Smoke")]
[Trait("Plugin", "LinkedIn")]
public class LinkedInHttpSmokeTests : ReliabilityTestBase, IClassFixture<GhostWebApiFixture>
{
    private readonly GhostWebApiFixture _fixture;

    public LinkedInHttpSmokeTests(GhostWebApiFixture fixture, ITestOutputHelper output) : base(output)
    {
        _fixture = fixture;
    }
    ...
    public async Task SearchJobs_Returns_Populated_Fresh_Data()
    {
        ...
        // Act
        Output.WriteLine($"Searching LinkedIn via API for: {searchRequest.query}");
    ...
    public async Task SearchJobs_WithLocation_Returns_Jobs_In_Location()
    {
        ...
        // Act
        Output.WriteLine($"Searching LinkedIn via API for: {searchRequest.query} in {searchRequest.location}");
        List<JobListing>? results = await _fixture.PostAsync<object, List<JobListing>>(
            "/api/jobs/search",
            searchRequest,
            Output);
