...
[Trait("Plugin", "Indeed")]
public class IndeedHttpSmokeTests : IClassFixture<GhostWebApiFixture>
{
    private readonly GhostWebApiFixture _fixture;
    private readonly ITestOutputHelper _output;

    public IndeedHttpSmokeTests(GhostWebApiFixture fixture, ITestOutputHelper output)
    {
        _fixture = fixture;
        _output = output;
    }
    ...
    public async Task SearchJobs_Returns_Populated_Fresh_Data()
    {
        ...
        // Act
        _output.WriteLine($"Searching Indeed via API for: {searchRequest.query}");
    ...
    public async Task SearchJobs_WithLocation_Returns_Jobs_In_Location()
    {
        ...
        // Act
        _output.WriteLine($"Searching Indeed via API for: {searchRequest.query} in {searchRequest.location}");
        List<JobListing>? results = await _fixture.PostAsync<object, List<JobListing>>(
            "/api/jobs/search",
            searchRequest,
            _output);
