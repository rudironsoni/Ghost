...
public static class TecnoempleoConstants
{
    ...
    // Spanish tech-specific salary patterns
    ...
    // Spanish tech job type patterns
    public static readonly Dictionary<string, JobType> JobTypeMapping = new()
    {
        ...
        ["proyecto"] = JobType.Contract
    };

    // Technology-specific keywords for Tecnoempleo
    public static readonly string[] TechnologyKeywords = new[]
    {
        "desarrollador",
        "programador",
        "ingeniero",
    ...
    public static bool IsTechnologyJob(string jobTitle)
    {
        if (string.IsNullOrWhiteSpace(jobTitle))
            return false;

        var lowerTitle = jobTitle.ToLower(CultureInfo.InvariantCulture);
        return TechnologyKeywords.Any(keyword => lowerTitle.Contains(keyword, StringComparison.OrdinalIgnoreCase));
    }
    ...
    public static string[] GetTechnologyKeywords() => TechnologyKeywords;
}
