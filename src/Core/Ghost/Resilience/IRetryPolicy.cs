...
public interface IRetryPolicy
{
    ...
    public Task<T> ExecuteAsync<T>(Func<Task<T>> action, Func<Exception, bool> isRetryable);
    ...
    /// <param name="action">The asynchronous action to execute.</param>
    /// <param name="isRetryable">Predicate that determines whether the exception is retryable.</param>
    public Task ExecuteAsync(Func<Task> action, Func<Exception, bool> isRetryable);

    /// <summary>
    /// Gets or sets the retry policy configuration options.
    /// </summary>
    public RetryPolicyOptions Options { get; set; }
