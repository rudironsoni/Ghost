...
public sealed class GlassdoorApiClient : IDisposable
{
    ...
    private async Task<string?> GetCsrfTokenWithOrchestratorAsync(CancellationToken ct)
    {
        try
        {
            ...
            try
            {
                ...
                try { await System.IO.File.WriteAllTextAsync("logs/glassdoor_csrf.html", html, ct).ConfigureAwait(false); } catch (Exception ex) { System.Diagnostics.Debug.WriteLine(ex, "$1"); }
                ...
                if (IsConsentOrBlockedPage(html))
                {
                    ...
                    try { await System.IO.File.WriteAllTextAsync("logs/glassdoor_csrf_alt.html", html, ct).ConfigureAwait(false); } catch (Exception ex) { System.Diagnostics.Debug.WriteLine(ex, "$1"); }
    ...
    /// </summary>
    private static async Task<string?> SearchWithSimpleHttpAsync(string keyword, string? location, CancellationToken ct)
    {
        try
        {
            ...
            try { await System.IO.File.WriteAllTextAsync("logs/glassdoor_simple_http.html", html, ct).ConfigureAwait(false); } catch (Exception ex) { System.Diagnostics.Debug.WriteLine(ex, "$1"); }
            ...
            {
                try { await System.IO.File.WriteAllTextAsync("logs/glassdoor_simple_http_parsed.json", jobsJson, ct).ConfigureAwait(false); } catch (Exception ex) { System.Diagnostics.Debug.WriteLine(ex, "$1"); }
        ...
        try
        {
            ...
            foreach (string? pattern in jsonPatterns)
            {
                ...
                if (match.Success && match.Groups.Count > 1)
                {
                    ...
                    catch (Exception ex) { System.Diagnostics.Debug.WriteLine(ex, "Job extraction failed"); }
                }
            }
    ...
    private async Task<string?> SearchWithOrchestratorAsync(string keyword, string? location, string? csrfToken, CancellationToken ct)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                try { await System.IO.File.WriteAllTextAsync($"logs/glassdoor_search.json", json, ct).ConfigureAwait(false); } catch (Exception ex) { System.Diagnostics.Debug.WriteLine(ex, "$1"); }
